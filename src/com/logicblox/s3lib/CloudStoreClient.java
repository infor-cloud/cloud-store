...
public interface CloudStoreClient {
	...
	/**
	 * Uploads a file according to {@code options}. For more details check
	 * {@link com.logicblox.s3lib.UploadOptions}.
	 * <p/>
	 * The specified bucket must already exist and the caller must have write
	 * permission to the bucket to upload an object.
	 * <p/>
	 * By default, the upload is multi-part with each part being
	 ...
	 * @see UploadOptions
	 */
	ListenableFuture<S3File> upload(UploadOptions options) throws IOException;

	/**
	 * Uploads the specified {@code file} under the specified {@code bucket} and
	 * {@code object} name. If the {@code key} is not null, the
	 * {@code keyProvider} will be asked to provide a public key with that name.
	 * This key will be used to encrypt the {@code file} at the client side. It
	 * applies {@code acl} permissions on the uploaded object. If the
	 * {@code kmsEncKey} is not null, The KmsClient will fetch the AWS KMS key
	 * and use it for a server side encryption.
	 *
	 * @param file
	 *            File to upload
	 * @param bucket
	 ...
	 * @param key
	 *            Name of encryption key to use
	 * @param kmsEncKey
	 *            Name of KMS encryption key to use
	 * @param acl
	 *            Access control list to use
	 * @see CloudStoreClient#upload(UploadOptions)
	 */
	@Deprecated
	ListenableFuture<S3File> upload(File file, String bucket, String object,
			String key, CannedAccessControlList acl) throws IOException;
	 ...
	 */
	@Deprecated
	ListenableFuture<S3File> upload(File file, URI uri) throws IOException;
	 ...
	 * {@code object} name. If the {@code key} is not null, the
	 * {@code keyProvider} will be asked to provide a public key with that name.
	 * This key will be used to encrypt the {@code file} at the client side.
	 * <p/>
	 * By default, the canned ACL "bucket-owner-full-control" is applied to the
	 * uploaded object.
	 *
	...
	/**
	 * Uploads a directory according to {@code options}. For more details check
	 * {@link com.logicblox.s3lib.UploadOptions}.
	 * <p/>
	 * Each individual file is uploaded with
	 * {@link CloudStoreClient#upload (UploadOptions)}.
	 * <p/>
	 ...
	 */
	@Deprecated
	ListenableFuture<List<S3File>> uploadDirectory(File directory, URI uri,
			String encKey) throws IOException, ExecutionException,
	...
	@Deprecated
	ListenableFuture<List<S3File>> uploadDirectory(File directory, URI uri,
			String encKey, CannedAccessControlList acl) throws IOException,
	 ...
	 * @param object
	 *            Object to be deleted from bucket
	 */
	...
	/**
	 * Downloads a file according to {@code options}. For more details check
	 * {@link com.logicblox.s3lib.DownloadOptions}.
	 * <p/>
	 ...
	 * indication that a file was uploaded by s3lib and/or the file can been
	 * fully downloaded using a single range GET.
	 * <p/>
	 * If during this call an exception wasn't thrown, the entire object has
	 * supposedly been stored successfully.
	 *
	 ...
	 * @param object
	 *            Path in bucket to download
	 * @see CloudStoreClient#download(DownloadOptions options)
	 */
	@Deprecated
	ListenableFuture<S3File> download(File file, String bucket, String object)
			throws IOException;
	 ...
	 *            File to download
	 * @param uri
	 *            Object URL to download from
	 * @see CloudStoreClient#download(DownloadOptions options)
	 */
	@Deprecated
	ListenableFuture<S3File> download(File file, URI uri) throws IOException;
	 ...
	 * <p/>
	 * If {@code recursive} is true, then all objects under {@code uri} will be
	 * downloaded. Otherwise, only the first-level objects will be downloaded.
	 * <p/>
	 * If {@code overwrite} is true, then newly downloaded files is possible to
	 * overwrite existing local files.
	 * <p/>
	 * Each individual file is downloaded with
	 * {@link CloudStoreClient#download(File, String, String)}.
	 *
	 * @param options
	 *            Download options
	 */
	 ...
	 * @param uri
	 *            Object URI that represents a directory (e.g. s3://bucket/key/)
	 * @param recursive
	 *            Download all files recursively under uri
	 * @param overwrite
	 *            Overwrite a file if it already exists
	 * @see CloudStoreClient#downloadDirectory(DownloadOptions options)
	 */
	@Deprecated
	ListenableFuture<List<S3File>> downloadDirectory(File directory, URI uri,
			boolean recursive, boolean overwrite) throws IOException,
			ExecutionException, InterruptedException;
	 ...
	 * in directories information too, then check
	 * {@link S3Client#listObjectsAndDirs}.
	 *
	 * @param bucket
	 *            Bucket to list objects from
	 * @param prefix
	 *            Prefix that all listed objects will start with
	 * @param recursive
	 *            Include objects of all subdirectories recursively
	 * @see CloudStoreClient#listObjectsAndDirs(String, String, boolean)
	 */
	ListenableFuture<List<S3ObjectSummary>> listObjects(String bucket,
	 ...
	 *            Bucket to list objects from
	 * @param prefix
	 *            Prefix that all listed objects and directories will start with
	 * @param recursive
	 *            Include objects of all subdirectories recursively
	 * @see CloudStoreClient#listObjects(String, String, boolean)
	 */
	ListenableFuture<List<S3File>> listObjectsAndDirs(String bucket,
			String prefix, boolean recursive);
