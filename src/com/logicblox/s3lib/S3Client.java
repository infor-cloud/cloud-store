...
public class S3Client implements CloudStoreClient {
	...
	/**
	 * Responsible for executing internal s3lib tasks asynchronously. Such tasks
	 * include file I/O, file encryption, file splitting and error handling.
	 */
	...
	/**
	 * Low-level AWS S3 client responsible for authentication, proxying and HTTP
	 * requests.
	 */
	 ...
	 *
	 * @param s3Client
	 *            Low-level AWS S3 client responsible for authentication,
	...
	public S3Client(AmazonS3Client s3Client) {
		this(s3Client, Utils.getHttpExecutor(10),
				Utils.getInternalExecutor(50), Utils.getDefaultChunkSize(),
	 ...
	 *
	 * @param credentials
	 *            The AWS credentials to use when making requests to the
	 *            service.
	 * @param s3Executor
	 *            Responsible for executing S3 HTTP API calls asynchronously.
	 *            It, also, determines the level of parallelism of an operation,
	 *            e.g. number of threads used for uploading/downloading a file.
	 * @param executor
	 *            Responsible for executing internal s3lib tasks asynchrounsly.
	 *            Such tasks include file I/O, file encryption, file splitting
	 *            and error handling.
	 * @param chunkSize
	 *            The size of an upload/download chunk (part) in bytes.
	 * @param keyProvider
	 ...
	 *      ListeningScheduledExecutorService, long, KeyProvider)
	 */
	public S3Client(AWSCredentialsProvider credentials,
			ListeningExecutorService s3Executor,
			ListeningScheduledExecutorService executor, long chunkSize,
	...
	public ListenableFuture<S3File> upload(UploadOptions options)
		...
		UploadCommand cmd = new UploadCommand(_s3Executor, _executor, file,
				_chunkSize, encKey, kmsEncKey, _keyProvider, acl,
	...
	// @Override
	public ListenableFuture<S3File> upload(File file, String bucket,
			String object, String key, String kmsEncKey,
			CannedAccessControlList acl) throws IOException {
		UploadOptions options = new UploadOptionsBuilder().setFile(file)
				.setBucket(bucket).setObjectKey(object).setEncKey(key)
				.setKmsEncKey(kmsEncKey).setAcl(acl.toString())
				.createUploadOptions();
	...
	public ListenableFuture<S3File> upload(File file, URI s3url)
			throws IOException {
		return upload(file, s3url, null, null);
	}

	// @Override
	public ListenableFuture<S3File> upload(File file, URI s3url, String key,
			String kmsEncKey) throws IOException {
		String bucket = Utils.getBucket(s3url);
		String object = Utils.getObjectKey(s3url);
		return upload(file, bucket, object, key, kmsEncKey);
	}

	@Override
	public ListenableFuture<S3File> upload(File file, String bucket,
			String object) throws IOException {
		return upload(file, bucket, object);
	}

	// @Override
	public ListenableFuture<S3File> upload(File file, String bucket,
			String object, String key, String kmsEncKey) throws IOException {
		UploadOptions options = new UploadOptionsBuilder().setFile(file)
				.setBucket(bucket).setObjectKey(object).setEncKey(key)
				.setKmsEncKey(kmsEncKey).createUploadOptions();
	...
	// @Override
	public ListenableFuture<List<S3File>> uploadDirectory(File directory,
			URI s3url, String encKey, String kmsEncKey) throws IOException,
			ExecutionException, InterruptedException {
		UploadOptions options = new UploadOptionsBuilder().setFile(directory)
				.setUri(s3url).setEncKey(encKey).setKmsEncKey(kmsEncKey)
				.setAcl("bucket-owner-full-control").createUploadOptions();
	...
	// @Override
	public ListenableFuture<List<S3File>> uploadDirectory(File directory,
			URI s3url, String encKey, String kmsEncKey,
			CannedAccessControlList acl) throws IOException,
			ExecutionException, InterruptedException {
		UploadOptions options = new UploadOptionsBuilder().setFile(directory)
				.setUri(s3url).setEncKey(encKey).setKmsEncKey(kmsEncKey)
				.setAcl(acl.toString()).createUploadOptions();
	...
	public ListenableFuture<S3File> delete(String bucket, String object) {
		DeleteCommand cmd = new DeleteCommand(_s3Executor, _executor);
		configure(cmd);
	...
	@Override
	public ListenableFuture<S3File> download(File file, String bucket,
			String object) throws IOException {
		DownloadOptions options = new DownloadOptionsBuilder().setFile(file)
				.setBucket(bucket).setObjectKey(object).createDownloadOptions();
	...
	public ListenableFuture<List<S3File>> downloadDirectory(
			DownloadOptions options) throws IOException, ExecutionException,
			InterruptedException {
		DownloadDirectoryCommand cmd = new DownloadDirectoryCommand(
				_s3Executor, _executor, this);
		configure(cmd);
	...
	@Override
	public ListenableFuture<List<S3File>> downloadDirectory(File directory,
			URI s3url, boolean recursive, boolean overwrite)
			throws IOException, ExecutionException, InterruptedException {
		DownloadOptions options = new DownloadOptionsBuilder()
				.setFile(directory).setUri(s3url).setRecursive(recursive)
				.setOverwrite(overwrite).createDownloadOptions();
	...
	public ListenableFuture<List<S3ObjectSummary>> listObjects(String bucket,
		...
		ListObjectsCommand cmd = new ListObjectsCommand(_s3Executor, _executor);
		configure(cmd);
	...
	public ListenableFuture<List<S3File>> listObjectsAndDirs(String bucket,
			String prefix, boolean recursive) {
		ListObjectsAndDirsCommand cmd = new ListObjectsAndDirsCommand(
				_s3Executor, _executor);
		configure(cmd);
		return cmd.run(bucket, prefix, recursive);
	}
	 ...
	 * "Version.RevNum-RevHash_BuildDatetime" </p> Example:
	 * "1.0.272-2dab7d1c9c69_201502181542"
	 */
	public static String version() {
		Package p = S3Client.class.getPackage();
		String v = p.getSpecificationVersion() + '.'
	...
	@Override
	public ListenableFuture<S3File> upload(File file, String bucket,
			String object, String key, CannedAccessControlList acl)
			throws IOException {
		UploadOptions options = new UploadOptionsBuilder().setFile(file)
				.setBucket(bucket).setObjectKey(object).setEncKey(key)
				.setKmsEncKey(kmsEncKey).setAcl(acl.toString())
	...
	@Override
	public ListenableFuture<S3File> upload(File file, String bucket,
			String object, String key) throws IOException {
		UploadOptions options = new UploadOptionsBuilder().setFile(file)
				.setBucket(bucket).setObjectKey(object).setEncKey(key)
				.setKmsEncKey(kmsEncKey).createUploadOptions();
	...
	@Override
	public ListenableFuture<List<S3File>> uploadDirectory(File directory,
			URI s3url, String encKey) throws IOException, ExecutionException,
			InterruptedException {
		UploadOptions options = new UploadOptionsBuilder().setFile(directory)
				.setUri(s3url).setEncKey(encKey).setKmsEncKey(kmsEncKey)
				.setAcl("bucket-owner-full-control").createUploadOptions();
	...
	@Override
	public ListenableFuture<List<S3File>> uploadDirectory(File directory,
			URI s3url, String encKey, CannedAccessControlList acl)
			throws IOException, ExecutionException, InterruptedException {
		UploadOptions options = new UploadOptionsBuilder().setFile(directory)
				.setUri(s3url).setEncKey(encKey).setKmsEncKey(kmsEncKey)
				.setAcl(acl.toString()).createUploadOptions();
