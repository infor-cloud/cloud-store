...
class Main
{
  ...
  public Main()
  {
    ...
    _commander.addCommand("list-pending-uploads", new ListPendingUploadsCommandOptions());
    _commander.addCommand("abort-pending-uploads", new AbortPendingUploadsCommandOptions());
    _commander.addCommand("exists", new ExistsCommandOptions());
    _commander.addCommand("list-buckets", new ListBucketsCommandOptions());
    _commander.addCommand("add-encryption-key", new AddEncryptionKeyCommandOptions());
    _commander.addCommand("remove-encryption-key", new RemoveEncryptionKeyCommandOptions());
  ...
  abstract class S3CommandOptions extends CommandOptions
  {
    ...
    @Parameter(names = "--stubborn", description = "Retry client exceptions (e.g. file not found " +
  ...
  class UploadCommandOptions extends S3ObjectCommandOptions
  {
    ...
    @Parameter(names = {"--chunk-size"}, description =
      "The size of each chunk read from the file. Determined " + "automatically if not set.")
    long chunkSize = -1;
  ...
  class ListCommandOptions extends S3ObjectCommandOptions
  {
    ...
    @Parameter(names = {"--exclude-dirs"}, description = "List only objects " +
      "(excluding first-level directories) that match the provided storage " + "service URL prefix")
  ...
  class DiskUsageCommandOptions extends S3ObjectCommandOptions
  {
    ...
    @Parameter(names = {"-d", "--max-depth"}, description = "Print sizes total for directories " +
      "with depth N")
    int maxDepth = 0;

    @Parameter(names = {"--all"}, description = "Print all files in directories " + "with depth N")
    boolean all = false;

    @Parameter(names = {"-H", "--human-readable-sizes"}, description =
  ...
  class AbortPendingUploadsCommandOptions extends S3ObjectCommandOptions
  {
    ...
    @Parameter(names = "--date", description = "Pending uploads older " +
      ...
      "8601 format \"yyyy-MM-dd\", UTC. Example: " + "\"2015-02-20\"")
    String dateStr;
    ...
    @Parameter(names = "--datetime", description = "Pending uploads older " +
      ...
      "ISO 8601 format \"yyyy-MM-dd HH:mm\", UTC. Example: " + "\"2015-02-20 19:31:51\"")
    String dateTimeStr;
  ...
  private String wrapDescription(int indent, String description, int columnSize)
  {
